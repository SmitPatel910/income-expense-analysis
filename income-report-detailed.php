...
?>
	...
	<input  type="button" id="btnExport" value="Download" onclick="Export()" />
		...
		<?php  
								...
								$var2 = array();
								$userid=$_SESSION['detsuid'];
								$ret=mysqli_query($con,"SELECT Type_Income,SUM(IncomeAmount) as ttl FROM `tbleincome`  where ((IncomeDate BETWEEN '$fdate' and '$tdate') && (UserId='$userid')) group by Type_Income");
